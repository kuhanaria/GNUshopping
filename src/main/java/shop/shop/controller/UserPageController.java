...
//
//        User user = userPageService.findUser(id);
//        Product product = productService.itemView(itemId);
//
...
//                if (cartItem.getProduct().getStock() == 0 || cartItem.getProduct().getStock() < cartItem.getCount()) {
//                    return "redirect:/main";
//                }
//                totalPrice += cartItem.getCount() * cartItem.getProduct().getPrice();
//            }
...
//
//                    // 판매자 수익 증가
//                    seller.setCoin(seller.getCoin() + (cartItem.getCount() * cartItem.getProduct().getPrice()));
//
//                    // 재고 감소
//                    cartItem.getProduct().setStock(cartItem.getProduct().getStock() - cartItem.getCount());
//
//                    // 상품 개별로 판매 개수 증가
//                    cartItem.getProduct().setCount(cartItem.getProduct().getCount() + cartItem.getCount());
//
//                    // sale, saleItem 에 담기
...
//            if (product.getStock() == 0 || product.getStock() < count) {
//                return "redirect:/main";
//            }
//
//            // 최종 결제 금액
...
//                product.getSeller().setCoin(product.getSeller().getCoin() + totalPrice);
//
...
//
//                // sale, saleItem 에 담기
//                SaleItem saleItem = saleService.addSale(product.getSeller().getId(), product, count);
//
//                // order, orderItem 에 담기
//                orderService.addOneItemOrder(user.getId(), product, count, saleItem);
//            }
//
